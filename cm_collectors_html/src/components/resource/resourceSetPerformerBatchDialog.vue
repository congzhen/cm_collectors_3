<template>
  <dialogCommon ref="dialogCommonRef" title="批量设置资源演员" width="700px" top="10vh" @submit="submitHandle">
    <div class="resource-set-performer" v-loading="loading">
      <div class="resource-set-performer-container">
        ...
        <div class="performer-container">
          <div class="performer-item" v-for="performer, key in performers" :key="key">
            <performerPopoverBlock :performer="performer">
...
const submitHandle = debounceNow(async () => {
  try {
    loading.value = true;
    const result = await resourceServer.batchSetPerformer(resourceIds, select_performers.value);
    if (result.status) {
